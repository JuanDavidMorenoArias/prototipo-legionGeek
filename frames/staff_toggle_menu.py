...
class StaffToggleMenuFrame(tk.Frame):
    ...
    def __init__(self,parent, log_out_callback, home_frame, user):
        super().__init__(parent,width=1000, background='#237F1A',
    ...
    def create_widgets(self):
        ...
        self.sidebar_button = tk.Button(self, text='☰', background='#237F1A', foreground='white',
                                    font=('Bold',20), border = 0, activebackground='#237F1A',
                                    activeforeground='white')
        self.sidebar_button.place(relx=0.025,rely=0.5, anchor='center')
        self.sidebar_button.config(command=self.open_bar)

        # Texto de "Hub"
        toggle_text = ttk.Label(self, text='Moderador', background='#237F1A', foreground='white', font=("Dubai Medium", 20))
...
class StaffSideBarFrame(tk.Frame):
    ...
    def __init__(self,parent,toggle_menu, home_frame, user):
         super().__init__(parent,width=200,height=530, background='#237F1A',
                         highlightbackground='white', highlightthickness=1)
         self.parent = parent
    ...
    def create_widgets(self): # Todos los botones pue 
        ...
        home_button = tk.Button(self, text='Inicio     🏡', font=("Dubai Medium", 16), background='#237F1A', foreground='white',
                                 border = 0, activebackground='#237F1A', activeforeground='white')
        ...
        logout_button = tk.Button(self, text='Cerrar Sesión🌏', font=("Dubai Medium", 16), background='#237F1A', foreground='white',
                                   border = 0, activebackground='#237F1A', activeforeground='white')
        logout_button.place(relx=0.13,rely=0.65)
        logout_button.config(command=self.log_out)

        exit_button = tk.Button(self, text='Salir    ❎', font=("Dubai Medium", 16), background='#237F1A', foreground='white',
                                 border = 0, activebackground='#237F1A', activeforeground='white')
